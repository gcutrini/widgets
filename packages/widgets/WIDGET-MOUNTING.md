...
This document is the contract for how legacy widgets are mounted in this app: a
...
(the app's React 19, in the page's tree) or `webComponent` (its own bundled
React 17, as a self-contained custom element).
...
- **What live data it needs** — realtime + auth + callbacks, bound each render.
  *Independent of how it runs.*
- **Which React runs it** — the app's React 19 (a component in the page's tree)
...
| `WidgetRenderer` (`renderAs`) | how it runs | `'react-component'` (app React) or `'web-component'` (own React) |
...
```
@openeventkit/widget-core   framework-free kernel — imported by BOTH the Next app and the esbuild build.
...
@openeventkit/widget-mount   React mount contract — <Widget>, WidgetRenderer, the renderer registry,
   ▲                         and the React-19 compat / prop-mutation-safety utilities. App-only.
@openeventkit/widgets        the uicore-bound part of each widget: manifest + vendor-styles.
   ▲                         (integration glue — compose/Client/index — lives in the app, src/widgets/<w>.)
src/components/widget/renderers/   the two concrete renderers; registered into widget-mount at startup.
```
...
@openeventkit/widget-core/
  ...
  host-auth.ts       HostAuth port — session presence + logout the app registers
  host-config.ts     HostConfig port — apiBaseUrl / idpBaseUrl / oauth2ClientId / timeApiUrl the app registers
  widget-auth-error.ts   the 401/403 DOM event the injected uicore auth handler raises and the app dialog handles
...
React-19 compat / prop-mutation-safety utilities. App-only — the esbuild bundle
never imports it (the bundle has its own kit).
...
@openeventkit/widget-mount/
  ...
  mutation-safe-props.ts   shallow-copy so a widget's in-place prop mutations can't reach app state
...
export function Widget({ manifest, composition, renderAs }: {
  ...
  const renderer = getRenderer(renderAs);   // filled by the app at startup
...
binding, the Client wrapper — is app integration glue and lives in the app under
`src/widgets/<widget>/`. See this package's [README](./README.md).

### 4 · `src/components/widget/renderers/` — the two renderers

The concrete renderers are Next/Sentry/uicore-coupled, so they stay in the app.
`register-host.ts` (imported in `Providers`) registers them into the widget-mount
...
- **`reactComponent`** — `createWidgetShadow(ref, manifest)`, then
  ...
  app's React 19, wrapped by `manifest.wrapTree`. widget-mount's shadow-react
  factory side-effect-imports the React-19 compat shims (`find-dom-node`,
  `react-element-symbol`); the app wrapper adds `compat/uicore-i18n`.
- **`webComponent`** — loads `${manifest.name}.shared.js` as an ES module,
  ...
  the app inlines (`WidgetImportMap`, first in the root layout's body) to the
...
and the HostAuth port (session presence + logout, from
...
The composer runs in the **app's React 19** for both renderers — the web-component
can't run app hooks inside its own React, so live state is always bound here and
handed across the boundary. Composers live with the rest of the integration glue
in `src/widgets/<widget>/compose.ts`.
...
The per-widget `Client.tsx` (in the app, `src/widgets/<widget>/`) calls the

...
describe('createShadowReactRenderer', () => {
  ...
  it('provides ShadowRootContext by default, without a host wrapShadowTree', async () => {
    const { ShadowRootContext } = await import('../../lib/context/shadow-root-context');
    const { useContext } = await import('react');
    let seen: ShadowRoot | null | undefined;
    const Probe = () => {
    ...
    expect(seen).toBe((host as Element | null)?.shadowRoot ?? seen);
  });
});

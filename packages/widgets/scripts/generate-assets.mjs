 ...
 * they reference.
 *
 * Vendor stylesheets are emitted as TypeScript modules under
 * `packages/widgets/src/kit/vendor-css/` (`export const sheet = { id, css,
 ...
 * Compared to serving copies from `public/`:
 *
 ...
 *   3. no generated CSS lives in public/ — only font/image binaries do
 *
 ...
 *
 * Runs via this package's `assets` script (the host's
 * `copy:widget-assets` calls it and syncs `assets/` into its public
 * dir). Outputs are checked into git so a fresh clone doesn't need to
 * run the script before typechecking.
 */
...
const vendorCssDir = join(pkgRoot, 'src/kit/vendor-css');
...
const cssTargets = [
  ...
  // itself → assets live under public/widget-css/ (see assetTargets).
  {
...
/**
 * Font-file source directories, copied wholesale into `public/fonts/`.
 * We flatten into one dir because filenames don't collide (glyphicons-*
 * vs fontawesome-*) and the rewritten CSS url()s point at `/fonts/x`.
 */
const fontSources = [
 ...
 * layout the rewritten URLs expect under `public/`.
 */
const assetTargets = [
...
function generateCssModule(specifier, outName, via) {
  ...
  const moduleSource = `// AUTO-GENERATED by packages/widgets/scripts/generate-assets.mjs — do not edit.
// Source: ${specifier}
